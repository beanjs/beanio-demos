...
const PIN_G = B1
const PIN_B = B2
const SUBJECT_COLOR = 'color-picker.rgb' // 数据通道名称，长度需小于32
...
// 设置 beanio 数据通道 事件函数
beanio.on(SUBJECT_COLOR, rgb => {
  const r = (rgb >> 16) & 0xff
  const g = (rgb >> 8) & 0xff
  const b = (rgb >> 0) & 0xff

  analogWrite(PIN_R, r / 255, { freq: 100 })
  analogWrite(PIN_G, g / 255, { freq: 100 })
  analogWrite(PIN_B, b / 255, { freq: 100 })
})
